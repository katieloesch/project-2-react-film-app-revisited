...
export default function TvCard({ item, setToWatchListElements,  setWatchedElements }) {
  const navigate = useNavigate();
  const [fetchedTvShows, setFetchedTvShows] = useState([])
  
  const { currentUser } = useAuth()
  const { toWatchList, setDetailsTitle, addNewToWatch, removeFromWatchList, checkIfonToWatchList, checkIfonWatched, selected, setSelected,
    markAsWatched, unMarkAsWatched, watched } = useContext(FilmTvContext);
...
async function handleUnmarkAsWatched() {
  ...
  async function handleImgClicked() {
    if (checkIfonToWatchList(item)) {
      setDetailsTitle(toWatchList.current.find(tv => tv.id === item.id))

    } else if (checkIfonWatched(item)) {
      setDetailsTitle(watched.current.find(tv => tv.id === item.id))
    } else {
      fetchTvShow()
...
if (item.poster_path){
  ...
  return (
    ...
    {(window.location.href.slice(-9) === 'watchlist' || window.location.href.slice(-7) === 'watched') && <div className='checkbox-container'><input type='checkbox' className='checkbox' id={`check-${item.id}`} onChange={handleCheckBoxChange}></input><label htmlFor={`check-${item.id}`}></label></div> }
